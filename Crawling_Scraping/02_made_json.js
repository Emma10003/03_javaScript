/*
require("파일 이름") : node_modules 중에서 fs 로 지칭되어 있는 파일 가져와서 사용
 require("fs");         Node.js 최초로 만들 때 기본적으로 내장되어 있는 기능
                        File System 에 줄임말
                        파일을 다룰 때 주로 사용
 require("puppeteer");  인형조종사를 의미하는 단어
                        웹 브라우저를 코드(인형) 처럼 조종하는 라이브러리
                        웹사이트 접속, 버튼 클릭, 텍스트 입력, 화면 스크린 샷, HTML 정보 가져오기 등
                        웹브라우저 자동화와 관련된 모든 기능 사용할 수 있음
                        npm install puppeteer 명령어로 미리 설치해야지만 require 로 node_modules 폴더에서
                        불러와 사용하기 가능
*/
...
지금은 async 를 제거해도 되지만, 여러 작업을 한 프로그램 안에 실행할 때는 필요
...
async function scrapeQuotes() {
    ...
    // fs.writeFileSync() : 파일에 작성하는 작업이 순차적으로 끝날 때까지
    // 다음작업은 시작하지 않고 기다린다.
    // .quote 내부에 작성된 데이터를 0번부터 순차적으로 작성하기 위해
    // 동기작업으로 파일 쓰기를 진행
    // writeFileSync() 단순 파일 저장 작업 할 때 주로 사용
    // writeFile()     비동기 형태로 파일 저장 수십 MB 넘는 큰 파일을 다룰 때는
    //                   프로그램 전체가 멈출 수 있기 때문에 비동기 방식인 writeFile() 사용

    /* 
 JSON.stringify(quotes, null, 2) 
 JSON.stringify()          :  객체나 배열을 문자열형태로 변환
                    quotes : 객체나 배열 대상으로 자바스크립트 변수이름에 저장되어 있는
                              목록을 파일명.json 으로 저장할 목록 리스트
 ...
 */
    ...
    // 개발자가 크롤링 후 json 으로 파일이 만들어졌는지 확인하기 위한 log 기록
