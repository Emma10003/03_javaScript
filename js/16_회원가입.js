...
// 그런데 만약에 모두 다 제대로 작성하지 않았다면 false 작성 제대로 안함 처리
// 값을 하나씩 제대로 작성할 대 마다 true로 변경해줄 것!
const checkList = {
    inputId: false, // input 내부에 개발자가 원하는 대로 아이디값을 제대로 작성했다면 true가 됨.
    inputPw: false, // input 내부에 개발자가 원하는 대로 비밀번호값을 제대로 작성했다면 true가 됨.
...
inputPw.addEventListener("input", (e) => {
    ...
    if (regExpPw.test(valPw)) {
        ...
        checkList["inputPw"] = true; // 비밀번호 형식이 일치한다면 위 작성한 checkList에서
...
function checkPw기능() {
    ...
    // pwVal이 0이거나       checkVal이 0인 게 둘 중 하나라도 참이라면
    if (pwVal.length == 0 || checkVal.length == 0) {
        ...
        // pwCheckResult 위치에 class 명칭을 추가하거나 제거하겠다. 4순위 style css
...
inputName.addEventListener("input", (e) => {
    ...
    if (valName.trim().length == 0) {
        ...
        e.target.value = ""; // e.target.value 대신 valName으로 써도 O.
        checkList["inputName"] = false;
        return; // 제대로 작성 안 했으니 아래 항목은 사용할 수 없도록 돌려보내기 설정
    }

    // 정규식 검사 (regExpName 사용)
...
btn.addEventListener("click", (e) => {
    ...
    // 비밀번호 값이 일치하지 않는 게 맞다면 (false가 맞다면)
    ...
    alert("회원가입이 완료되었습니다.");
    signupForm.submit(); // 회원가입 폼 제출완료 기능 설정
});
