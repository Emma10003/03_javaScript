...
function loginFn() {
    ...
    if (!storedUserJSON) {
        ...
        window.location.href = "31_회원가입.html";
        return;
    }
    ...
    //    참고 : 추후에는 DB에 저장된 이메일값과 사용자가 서버로 보낸 이메일값이
    //          일치하는지 여부에 대해서는 백엔드에서 작업처리를 진행한 후 프론트엔드로 전달
    //          현재는 연결된 DB가 존재하지 않기 때문에 프론트엔드에서 모든 작업을 진행하는 것!
    if (userEmail === storedUser.email) {
        // 5. 소비자가 작성한 이메일값과 저장된 이메일이 일치한다면
        ...
        // 웹사이트 session에 자바스크립트에서 작성한 로그인 정보를 저장ㅋ
